...
# Opens ai3.storage in your browser, prompts for the API key,
...
DASHBOARD_URL="https://ai3.storage"
echo "Opening $DASHBOARD_URL in your browser..."
echo "  → Sign in with Google or GitHub"
echo "  → Navigate to Developers → Create API Key"
echo ""
...
if [[ "$HTTP_CODE" -lt 200 || "$HTTP_CODE" -ge 300 ]]; then
  ...
  echo "$BODY" >&2
  echo "Double-check the key at $DASHBOARD_URL and try again." >&2
  exit 1
fi
